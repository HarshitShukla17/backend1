...
const toggleSubscription = asyncHandler(async (req, res) => {
    ...
    if(!channel)
    {
        throw new ApiError(404,"channel does not exists for given channelId")
    }
    const loggedInUser=req.user?._id

    const userSubscribed=await Subscription.findOneAndDelete({
        subscriber:loggedInUser,channel:channel._id
    })
    if(userSubscribed)
    {
        return res
    ...
    if(!userSubscribed)
    {
        const subscription=await Subscription.create({
            subscriber:loggedInUser,channel:channelId
        })
        if(!subscription)
        {
        ...
        return res
        .status(200)
        .json(new ApiResponse(200,subscription,"users subscribed the channel successfully"))
    }
})
